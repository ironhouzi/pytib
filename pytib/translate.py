from pytib.tables import consonants, PREFIXES_I, get_chars
...
def validSubscribe(root_letter, subjoined_letter, subjoined, valid_subjoined):
    return (
        subjoined_letter in subjoined
        and root_letter in valid_subjoined[subjoined_letter]
    )


def validate(wylieLetters, vowelPosition, sub=True):
    """ Docstring
    sub is 1 if checking subscribed and 0 if checking superscribed
    """
    # truth_vector = [x % 2 == sub for x in range(4)]
    import itertools
    truth = itertools.cycle([sub, not sub])

    if vowelPosition == 2:
        return (
            not next(truth) ^ validSuperscribe(wylieLetters[0], wylieLetters[1])
            and not next(truth) ^ validSubscribe(wylieLetters[0],
                                                 wylieLetters[1])
        )
    else:   # vowelPosition == 3
        return (
            wylieLetters[0] in PREFIXES
            and not next(truth) ^ validSuperscribe(wylieLetters[1],
                                                   wylieLetters[2])
            and not next(truth) ^ validSubscribe(wylieLetters[1],
                                                 wylieLetters[2])
        )
